...
const newHtml = fs.createWriteStream('06-build-page/project-dist/template.html')
html.pipe(newHtml)
fs.readFile('06-build-page/project-dist/template.html','utf-8',(err,data) => {
    if (err) throw err
    fs.readFile('06-build-page/components/header.html','utf-8',(err,data2)=> {
        if (err) throw err
        data = data.replace("{{header}}",data2)
        fs.writeFile('06-build-page/project-dist/template.html',data,'utf-8',err => {
            if (err) throw  err
        })
    })
    fs.readFile('06-build-page/components/articles.html','utf-8',(err,data3)=> {
        if (err) throw err
            ...
            if (value.isFile()) {
                fs.readFile(`${dir}/${el.name}/${value.name}`,'utf-8',(err,data)=> {
                    if (err) throw err
                        ...
                        fs.writeFile(`${dir2}/${el.name}/${value.name}`,data,'utf-8',(err)=> {
                            if (err) throw  err
                            else console.log('ok')
                        })
                    }
                ...
                fs.mkdir(`06-build-page/project-dist/assets/${value.name}`, {recursive: true}, (err) => {
                    if (err) throw  err
                })
                ...
                fs.readdir(dir3,{withFileTypes: true},(err,data) => {
                    if (err) throw  err
                    const dir2 = path.join(__dirname, `project-dist/assets/${value.name}`)
                    data.forEach((elem)=> {
                        recursive(elem)
                    })
                })
